...
float OmniDirectionalShadowFactor(PointLight light, int shadowIndex)
{
    vec3 fragToLight = v_fragPos - light.position;
    float closest = texture(u_omniShadowMaps[shadowIndex].shadowMap, fragToLight).r;

    // depth is between 0 and 1 as in fragment shadder. Scale it back up.
    closest *= u_omniShadowMaps[shadowIndex].farPlane;
...
vec4 CalculatePointLight(PointLight pointLight, int index)
{
    vec3 direction = v_fragPos - pointLight.position;
    float distance = length(direction);

    float shadowFactor = OmniDirectionalShadowFactor(pointLight, index);

    vec4 color = CalculateDirectionalLight(pointLight.base, direction, shadowFactor);
...
vec4 GetPointLights()
{
    ...
    {
        finalColor += CalculatePointLight(u_pointLights[i], i);
    }
    return finalColor;
}
...
vec4 GetSpotLights()
{
    ...
    for (int i = 0; i < u_spotLightsCount; i++)
    {
        ...
        {
            finalColor += CalculatePointLight(spotLight.base,i) * (1.0f - (1.0f - slFactor) * (1.0f / (1.0f - spotLight.edge)));
        }
    }
    return finalColor;
}
