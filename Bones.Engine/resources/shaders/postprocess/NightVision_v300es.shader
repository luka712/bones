...
uniform float u_scanLineTileAmount;
...
uniform vec3 u_nightVisionColor;
...
void main()
{
	...
	lum += u_brightness;
	// TODO: 
	color = lum * 2.0 + vec4(u_nightVisionColor, 1.0);
...
vec4 noiseEffect()
{
	vec2 noiseUV = vec2(v_texCoords.x + (u_random * sin(u_time * 0.1) * u_noiseSpeed.x), v_texCoords.y + (u_time * u_noiseSpeed.y));
